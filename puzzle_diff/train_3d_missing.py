...
def main(
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor="rmse_r", mode="min", save_top_k=2, save_last=True
    )
    ...
    if evaluate:
        for _ in range(num_iter):
            ...
            results.append(list(result[0].values())[-4:])
            del model
        results = np.array(results)
        print('Mean:', results.mean(axis = 0))
        print('Standard Deviation:' ,results.std(axis = 0))
        print('---')
    else:
...
if __name__ == "__main__":
    ...
    ap.add_argument('--num_iter', type=int, default=1)
